...
// https://colab.research.google.com/drive/1FgTVX4aRaE-78Oymd4cfOaf296vCfK_Y?usp=sharing
...
const int led_1 = 19, led_2 = 25, led_3 = 15;

String label[] = { "LED 2", "Off", "On", "LED 1", "LED 3" };

int label_value[] = { led_2, LOW, HIGH, led_1, led_3 };
int sellect_pin = led_1;

#include <driver/i2s.h> // Hardware i2s module
#define bck_io_pin   17     //in3
#define ws_io_pin    4     //out1
#define data_in_pin  5     //out2
...
void loop() {
    ...
    for (int k = 1; k < real_fft_plan->size / 2; k++)
    {
        ...
        if (display_mode == 1) {
            ...
            y = 0;
            if (x * 2 < 160) {
                tft.fillRect(x * 2, 0, 2, 80 - (mag), TFT_BLACK);
                tft.fillRect(x * 2, 80 - (mag)+1, 2, (mag), mag_2_color(mag * 5));
                x++;
            }
        }
    }
    ...
    if (percent_max >= 0.9) {
        ...
        if (label_index == 1 || label_index == 2) {

            digitalWrite(label_value[sellect_pin], label_value[label_index]);

            sprintf(print_buf, "%s  : %s", label[sellect_pin], label[label_index]);
            tft.setCursor(12, 12);
            tft.print(print_buf);

        }
        else {
            sellect_pin = label_index;

            sprintf(print_buf, "Select : %s", label[label_index]);
            tft.setCursor(12, 12);
            tft.print(print_buf);
        }
